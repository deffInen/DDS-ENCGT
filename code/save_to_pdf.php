...
$pdf_name = 'dds_annonce_' . $announceid . '.pdf';

// Set the path to the PDF file
$pdf_path = '/var/www/html/DDS/DDS-ENCGT/code/announces/' . $pdf_name;

// Check if the PDF file already exists
if (file_exists($pdf_path)) {
  // If the PDF file exists, display it to the user
    ...
    <div class="main-content-container-dds">
      <div class="info-content-container-dds">
        <div class="top-logos">
          ...
          <img src="data:' . $annonce_obj->entreprise_info->entreprise_logo->entreprise_logo_file_type . ';base64,' . $annonce_obj->entreprise_info->entreprise_logo->entreprise_logo_file_content . '" alt="logo_en">
          </div>
        </div>
...
';
  ...
  // Save the PDF to a file in the "announces" folder
  $file_path = '/var/www/html/DDS/DDS-ENCGT/code/announces/' . $pdf_name;
  file_put_contents($file_path, $pdf_content);

  // Display the generated PDF to the user
  header('Content-Type: application/pdf');
  header('Content-Disposition: inline; filename="' . $pdf_name . '"');
  header('Content-Transfer-Encoding: binary');
  header('Accept-Ranges: bytes');
